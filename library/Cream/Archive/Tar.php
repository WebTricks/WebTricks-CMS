 ...
 */
class Cream_Archive_Tar extends Cream_Archive_Abstract implements Cream_Archive_Interface
{
    /**
    ...
    protected function _packToTar($skipRoot=false)
    {
        ...
        if (is_dir($file)) {
            ...
            if (empty($treeDir)) {
                throw new Cream_Exceptions_Exception('Can\'t scan dir: ' . $file);
            }
    ...
    protected function _unpackCurrentTar($destination)
    {
        ...
        if (empty($pointer)) {
            throw new Cream_Exceptions_Exception('Can\'t open file: ' . $file);
        }
        $list = array();
        while (!feof($pointer)) {
    ...
    public function pack($source, $destination, $skipRoot=false)
    {
        ...
        $tarData->_setCurrentFile($source);
        $tarData->_packToTar($skipRoot);
    ...
    public function extract($file, $source, $destination)
    {
        ...
        if (empty($pointer)) {
            throw new Cream_Exceptions_Exception('Can\'t open file: '.$source);
        }
        $list = array();
        $extractedFile = '';
        while (!feof($pointer)) {
