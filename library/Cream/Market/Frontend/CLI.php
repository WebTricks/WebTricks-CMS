 ...
 */
class Cream_Market_Frontend_CLI extends Cream_Market_Frontend
{
    /**
     * Collected output
	 * 
    ...
    public function outputListChannels($data)
    {
        ...
        $channels =& $data['data'][Cream_Market_Singleconfig::K_CHAN];
        foreach($channels as $name => $v) {
            $this->writeln("$name: {$v[Cream_Market_Singleconfig::K_URI]}");
        }
        $aliases =& $data['data'][Cream_Market_Singleconfig::K_CHAN_ALIAS];
        if(count($aliases)) {
            $this->writeln();
     ...
     * @return void
     */
    public function outputPackage(Cream_Market_Package $package)
    {
        $fields = array(
            'Name'=>'name',
            'Version'=>'version',
