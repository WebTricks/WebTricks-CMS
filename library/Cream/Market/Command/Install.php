...
final class Cream_Market_Command_Install extends Cream_Market_Command
{
    /**
    ...
    public function doInstall($command, $options, $params, $objects = array())
    {
        ...
        try {
            ...
            if($installFileMode) {
                ...
                $package = Cream_Market_Package::instance($filename);
                $package->validate();
            ...
            foreach($packagesToInstall as $package) {
                try {
                    ...
                    if ($upgradeMode && !$ignoreModifiedMode) {
                        ...
                        if (count($modifications) > 0) {
                            ...
                            foreach ($modifications as $row) {
                                if(!$ftp) {
                                    $this->ui()->output($config->webtricks_root.DS.$row);
                                } else {
                                    $this->ui()->output($row);
                                }
                            }
                    ...
                    $package = Cream_Market_Package::instance($file);
