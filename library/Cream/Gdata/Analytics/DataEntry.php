<?php
class Cream_Gdata_Analytics_DataEntry extends Zend_Gdata_Entry {
    ...
    protected function takeChildFromDOM($child)
    {
        ...
        switch ($absoluteNodeName){
        	case $this->lookupNamespace('ga') . ':' . 'dimension';
	            $dimension = new Cream_Gdata_Analytics_Extension_Dimension();
	            $dimension->transferFromDOM($child);
	            $this->_dimensions[] = $dimension;
            break;
        	case $this->lookupNamespace('ga') . ':' . 'metric';
	            $metric = new Cream_Gdata_Analytics_Extension_Metric();
