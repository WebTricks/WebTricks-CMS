...
class WebTricks_Shell_Commands_SetDisplayName extends WebTricks_Shell_Commands_Command
{	
	/**
	...
	public function execute(WebTricks_Shell_Commands_CommandContext $context)
	{
		if ($context->hasResult()) {
			if ($context->getResult() == 'ok' && count($context->getItems()) == 1 && $context->getItem()->getAccess()->canWrite()) {
				...
				$context->getItem()->getAppearance()->setDisplayName($context->getValue());
				$context->getItem()->getEditing()->endEdit();
			}
		} else {
			WebTricks_Shell_Client_Response::prompt('Enter a new display name for the item:');
		}
	}
