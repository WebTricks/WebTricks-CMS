...
class WebTricks_Shell_Commands_Item_CopyToClipboard extends WebTricks_Shell_Commands_Command
{
	public function execute(WebTricks_Shell_Commands_CommandContext $context)
	{
		if (count($context->getItems()) == 1) {
			$session = WebTricks_Shell_Session::singelton();
			$session->setClipboard('copy:'. $context->getItem()->getItemId());
		}
	}
